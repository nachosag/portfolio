...
test.describe('Portfolio', () => {
  ...
  test('typewriter activates on hero scroll',
  ...
  test('project filter toggles cards by category',
    { tag: ['@critical', '@interaction'] },
    async () => {
      const allBtn = portfolio.filterBar.getByRole('button', { name: 'ALL', exact: true });
      const backendBtn = portfolio.filterBar.getByRole('button', { name: 'BACKEND', exact: true });
      ...
      const visibleAfterFilter = await portfolio.explorerGrid.locator('> div').evaluateAll(
      ...
      const visibleAfterAll = await portfolio.explorerGrid.locator('> div').evaluateAll(
        (els) => els.filter(el => (el as HTMLElement).style.display !== 'none').length
      );
  ...
  test('mobile menu opens and closes',
    ...
    async () => {
      ...
      await expect(portfolio.mobileMenuBtn).toHaveAttribute('aria-expanded', 'true');
      ...
      await expect(portfolio.mobileMenuBtn).toHaveAttribute('aria-expanded', 'false');
