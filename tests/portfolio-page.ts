...
export class PortfolioPage {
  ...
  async clickFilter(category: string) {
    await this.filterBar.getByRole('button', { name: category, exact: true }).click();
    await this.page.waitForTimeout(200);
  }

  async getVisibleCards(): Promise<Locator> {
    return this.explorerGrid.locator('> div').filter({ hasNot: this.page.locator('[style*="display: none"]') });
  }
